...
const AIOrb: React.FC<AIOrbProps> = ({
  ...
  return <button id="ai-orb-button" onClick={handleClick} className={`relative w-12 h-12 rounded-full transition-all duration-300 cursor-pointer group ${isOrbFocused ? 'ring-2 ring-white' : ''} ${orbState === 'active' ? 'bg-gradient-to-r from-green-500 to-green-600 scale-110 shadow-lg shadow-green-500/50' : orbState === 'muted' ? 'bg-gradient-to-r from-yellow-500 to-orange-600 scale-110 shadow-lg shadow-yellow-500/50' : 'bg-gradient-to-r from-gray-600 to-gray-700 hover:from-psyco-green-DEFAULT/80 hover:to-blue-500/80'}`}>
      ...
      {/* Content */}
      <div className="absolute inset-0 flex items-center justify-center py-0 mx-0 my-[3px]">
        {orbState === 'active' ? <div className="flex items-center space-x-0.5">
            {[...Array(5)].map((_, i) => <div key={i} className="w-0.5 bg-white rounded-full animate-pulse transition-all duration-75" style={{
